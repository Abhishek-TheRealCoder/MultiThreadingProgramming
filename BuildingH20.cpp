...
// given releaseHydrogen and releaseOxygen methods respectively, which will allow them to pass the barrier. 
...
public:
    ...
    bool isHydrogenTurnDone;
    H2O() {
        isHydrogenTurnDone = false;
        countH = 0;
    }

    void hydrogen(function<void()> releaseHydrogen) {
        unique_lock<mutex>lock(mx);
        while(isHydrogenTurnDone)
        {
            cv.wait(lock);
        }
